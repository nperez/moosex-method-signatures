...
sub _parse_prototype_injections {
    ...
    my @params;
    foreach my $inject (@{ $self->prototype_injections })
    {   
        my $param;
...
sub _build__lexicals {
    ...
    {
        push(@lexicals, $_->variable_name) for @{ $self->_parsed_prototype_injections };
    }
...
sub _build__positional_args {
    ...
    if($self->_has_parsed_prototype_injections)
    {
        push(@positional, map { $self->_param_to_spec($_) } @{ $self->_parsed_prototype_injections });
    }
